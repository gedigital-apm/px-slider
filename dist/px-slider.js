'use strict';(function(){'use strict';Polymer({is:'px-slider',properties:{/**
      ...
      */endValue:{type:Number,notify:true,value:null},/**
       * The display value for the left input box
       */formattedStartValue:{type:String},/**
       * The display value for the right input box
       */formattedEndValue:{type:String},/**
       ...
       */_minMaxValid:{type:Number,value:0},/**
      ...
      */step:{type:Number,value:1,observer:'_stepChanged'},/**
       ...
       * - `logarithmic`
       * - `exponential`
       */scale:{type:String,value:'linear',observer:'_createScale'},/**
       ...
       */showLabels:{type:Boolean,value:false},/**
       * The vertical location for the Min label. Valid values are:
       * - 'bottom' (default)
       * - 'top'
       */minLabelPosition:{type:String,value:'bottom'},/**
       ...
       */_handleDefinitions:{type:Object,value:function value(){return{'circle':{'bodyD':'m0,-7.5c5,0 10,5 10,10c0,5 -5,10 -10,10c-5,0 -10,-5 -10,-10c0,-5 5,-10 10,-10z'},'down':{'bodyD':'m7.5,-13l-15,0l7.5,12l7.5,-12z'},'up':{'bodyD':'m7.5,18l-15,0l7.5,-12l7.5,12z'}}}}},behaviors:[Polymer.IronResizableBehavior,PxNumberFormatter.commonProperties],listeners:{'iron-resize':'_onIronResize'},observers:['_minOrMaxChanged(min, max)','_setRange(_scale, _width, _height)','_setDomain(_scale, _minMaxValid)','_valueChanged(value, _scale, _scaleChanged)','_endValueChanged(endValue, _scale, _scaleChanged)','_updateFormat(step)','_updateFormat(format)','_isRangeChanged(isRange)','_hideInputsChanged(hideInputs)'],ready:function ready(){window.requestAnimationFrame(this._animationFrame.bind(this))},attached:function attached(){this.setAttribute('role','slider')},/**
...
this._setupListeners()},/**
     * Setup our listeners
     */_setupListeners:function _setupListeners(){if(this._startHandle&&this._endHandle){if(this.disabled){this._removeHandleListeners();this._removeTrackListeners()}else{// create our drag and hover,etc  states
this._createHandleListeners(this._startHandle,'value');this._createHandleListeners(this._endHandle,'endValue');// setup click listeners on the track and bars
this._createTrackListeners()}// add or remove disabled class
this._toggleDisabledClass()}},/**
     * Creates the drag behavior and hover, pressed states for the handles.
     */_createHandleListeners:function _createHandleListeners(handle,valueVar){var handleBody=handle.select('.handleBody');var progressBar=Px.d3.select(this.$$('.progressBar'));handle.call(Px.d3.drag().on('start.interrupt',function(){handle.interrupt()}).on('start drag',function(){// apply styling
handleBody.classed('handleBodyPressed',true);progressBar.classed('progressBarPressed',true);// TODO for vertical: check state, pass in d3.event.y
this._calcSliderValue(Px.d3.event.x,valueVar)}.bind(this)).on('end',function(){// remove styling
handleBody.classed('handleBodyPressed',false);progressBar.classed('progressBarPressed',false)})).on('mouseenter',function(){handleBody.classed('handleBodyHover',true);progressBar.classed('progressBarHover',true)}).on('mouseleave',function(){handleBody.classed('handleBodyHover',false);progressBar.classed('progressBarHover',false)})},/**
     * Creates the click listeners on the progress bars to enable click to jump value
     */_createTrackListeners:function _createTrackListeners(){var _this=this,interactionRects=Px.d3.selectAll(Polymer.dom(this.$$('#sliderSVG')).querySelectorAll('.sliderTrack'));interactionRects.on('click',function(){// need access to callback this, so pass it in
_this._trackOnClick(this)})},/**
     * The actual callback for our track on click
     */_trackOnClick:function _trackOnClick(elem){// TODO vertical: [1]
var val=Px.d3.mouse(elem)[0],// Get the mouse position
prop='value';//assume it should affect the left value
if(this.isRange){//check to see which is closer
var scaled=this._scale.invert(val),half=(this.endValue-this.value)/2+this.value;prop=scaled>this.endValue||scaled>half?'endValue':'value'}this._calcSliderValue(val,prop)},/**
     * Clears the listeners from the handles
     */_removeHandleListeners:function _removeHandleListeners(){this._startHandle.on('.drag',null);this._endHandle.on('.drag',null)},/**
     * clears the listeners from the track
     */_removeTrackListeners:function _removeTrackListeners(){var interactionRects=Px.d3.selectAll(Polymer.dom(this.$$('#sliderSVG')).querySelectorAll('.sliderTrack'));interactionRects.on('click',null)},/**
     ...
     */_buildHandles:function _buildHandles(){if(this._handleDefinitions&&this._startHandle&&this._endHandle){if(this.isRange){this._startHandle.select('.handleBody').attr('d',this._handleDefinitions.down.bodyD);this._endHandle.select('.handleBody').attr('d',this._handleDefinitions.up.bodyD)}else{this._startHandle.select('.handleBody').attr('d',this._handleDefinitions.circle.bodyD);// remove end handle:
this._endHandle.select('.handleBody').attr('d',null)}}},/**
     * saves our input fields as a property for conveinence
     */_assignInputElems:function _assignInputElems(){// make  sure those dom-ifs have run
Polymer.dom.flush();this.set('_inputStart',this.$$('#inputStart'));this.set('_inputEnd',this.$$('#inputEnd'))},/**
     * Resizes the slider based on parent box size
     */_onIronResize:function _onIronResize(){// TODO enable this?
// if(this.preventResize) {
//   return;
// }
this.debounce('ironresize',function(){// TODO vertical version
...
this._scaleChanged=!this._scaleChanged},/**
     ...
     */_calcSliderValue:function _calcSliderValue(mouseVal,valueVar){this.debounce('_calcSliderValue',function(){this._calcSliderValueDebounced(mouseVal,valueVar)},10)},/**
     * Calculate the slider handle value for realz
     */_calcSliderValueDebounced:function _calcSliderValueDebounced(mouseVal,valueVar){var val=this._scale.invert(mouseVal),//convert pixel --> value
...
if(this.isRange){stepped=valueVar==='value'&&stepped>this.endValue?this.endValue:stepped;stepped=valueVar==='endValue'&&stepped<this.value?this.value:stepped}this.toggleClass('validation-error',false,inputElem);this.set(valueVar,stepped)},/**
     * Rounds the raw pixel translated value to one that matches our step size
     */_calcStepRounded:function _calcStepRounded(value){var s=Math.round(value/this.step)*this.step;// make sure it isnt rounding out of our range; should only do so by one step:
s=s<this.min?s+this.step:s;s=s>this.max?s-this.step:s;return s},/**
     * Validate the value
     */_validateValue:function _validateValue(v,thisVal){var valid=true;// check that value <= endValue
if(this.isRange&&thisVal==='value'&&v>this.endValue){v=this.endValue;valid=false;this.set(thisVal,v);// check that endValue <= vale
}else if(this.isRange&&thisVal==='endValue'&&v<this.value){v=this.value;valid=false;this.set(thisVal,v)}// check v is greater than the min
if(v<this.min){v=this._calcStepRounded(this.min);valid=false;this.set(thisVal,v);// check v is less than the max
     ...
     */_calcProgressEnd:function _calcProgressEnd(){if(this.isRange){return Math.max(this._scale(this.endValue)-this._scale(this.value),1)}return this._scale(this.value)},/**
     * Updates the formating string for nubmer-formatter
     */_updateFormat:function _updateFormat(){// if dev passed in a format, just use it
...
var s=this.step.toString().split('.'),l=s.length===2?s[1].length:0,f='0.';for(var i=0;i<l;i++){f+='0'}this.set('_format',f)},/**
     ...
     */_inputChangedEnd:function _inputChangedEnd(evt){var text=this._inputEnd.value;this._inputChanged(text,this._inputEnd,'#formatterEnd','endValue')},/**
     * When an input field has changed, validates the input and sets the value property
     */_inputChanged:function _inputChanged(text,inputElem,formatterId,valueVar){var formatter=this.$$(formatterId),newVal;// get a raw number from our input field string
formatter.set('unformat',text);newVal=formatter.unformattedValue;if(!newVal&&newVal!==0){// toggle Error state
this.toggleClass('validation-error',true,inputElem);return}// make sure our newVal is between our max and min
// TODO Check with design if this is the appropriate approach: num sets to min or max rather than toggling validation error
newVal=Math.max(newVal,this.min);newVal=Math.min(newVal,this.max);// make sure it matches our step
newVal=this._calcStepRounded(newVal);// make sure it is not above/below the other value if range
if(this.isRange){if(valueVar==='value'&&newVal>this.endValue||valueVar==='endValue'&&newVal<this.value){this.toggleClass('validation-error',true,inputElem);return}}this.toggleClass('validation-error',false,inputElem);/*
      ...
      */this[valueVar]=null;this.set(valueVar,newVal)},/**
     * If the user hits ESC on an input field, reset the display value
     */_restoreStartInputValue:function _restoreStartInputValue(evt){this._restoreInputValue(this._inputStart,this.formattedStartValue)},/**
     * If the user hits ESC on an input field, reset the display value
     */_restoreEndInputValue:function _restoreEndInputValue(evt){this._restoreInputValue(this._inputEnd,this.formattedEndValue)},/**
     * Reset the display value in the input field to the coresponding value property
     */_restoreInputValue:function _restoreInputValue(input,formattedVal){// must blur for FF to reset value
input.blur();input.value=formattedVal;this.toggleClass('validation-error',false,input)},/**
